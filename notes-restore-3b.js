(function(){
  ...
  async function boot(){
    ...
    for(const [id,v] of Object.entries(NOTES)){
      const s=db.students.find(x=>String(x.id)===id);if(!s)continue;
    ...
    if(changed){try{await window.save();window.renderGrades&&window.renderGrades();window.toast&&toast('Notas do 3º B — Português — 2º bimestre salvas.')}catch(e){console.error(e)}}
